...
const mockFriends             = [{ id: 5, firstName: "David", lastName: "Wilson", email: "david@example.com" }];
const mockSentInvitations     = [{ id: 1, recipient_id: 1, firstName: "Emma", lastName: "Watson", email: "emma@example.com", status: "pending" }];
const mockReceivedInvitations = [{ id: 2, sender_id: 6, firstName: "Frank", lastName: "Miller", email: "frank@example.com", status: "pending" }];
...
const setup = async (overrides = {}) => {
  mockApi(overrides);
  render(<BrowserRouter><Profil /></BrowserRouter>);
...
const setupEmpty = async () => {
  mockApi({ getAllUsers: [mockConnectedUser], "friends": [], "invitations/sent": [], "invitations/received": [] });
  render(<BrowserRouter><Profil /></BrowserRouter>);
  const user = userEvent.setup();
  await waitFor(() => expect(screen.getByText("Aucun utilisateur trouvé")).toBeInTheDocument(), { timeout: 3000 });
  return user;
};
...
describe("Profil Component", () => {
  ...
  test("filtre par recherche", async () => {
    const user = await setup({ "friends": [], "invitations/sent": [], "invitations/received": [] });
    await user.type(screen.getByPlaceholderText("Filtrer les utilisateurs..."), "Alice");
    await waitFor(() => expect(screen.queryByText("Bob Johnson")).not.toBeInTheDocument(), { timeout: 3000 });
  ...
  test("onglet Amis → affiche les amis", async () => {
    ...
    await waitFor(() => expect(screen.getByText("David Wilson")).toBeInTheDocument(), { timeout: 3000 });
  ...
  test("onglet Invitations Envoyées → affiche Emma Watson", async () => {
    ...
    expect(tab).toHaveClass("active");
    await waitFor(() => expect(screen.getByText("Emma Watson")).toBeInTheDocument(), { timeout: 3000 });
  });
  ...
  test("onglet Invitations Reçues → affiche Frank Miller", async () => {
    ...
    expect(tab).toHaveClass("active");
    await waitFor(() => expect(screen.getByText("Frank Miller")).toBeInTheDocument(), { timeout: 3000 });
  });

  test("états vides sur tous les onglets", async () => {
    const user = await setupEmpty();
    await clickTab(user, /👥 Amis \(0\)/i);
    await waitFor(() => expect(screen.getByText("Aucun ami pour le moment")).toBeInTheDocument(), { timeout: 3000 });
    await clickTab(user, /📤 Invitations Envoyées \(0\)/i);
    await waitFor(() => expect(screen.getByText("Aucune invitation envoyée")).toBeInTheDocument(), { timeout: 3000 });
    await clickTab(user, /📥 Invitations Reçues \(0\)/i);
    await waitFor(() => expect(screen.getByText("Aucune invitation reçue")).toBeInTheDocument(), { timeout: 3000 });
  });

  test("ouvre le modal utilisateur", async () => {
    const user = await setup({ "friends": [], "invitations/sent": [], "invitations/received": [] });
    await user.click(screen.getAllByText("+")[0]);
    await waitFor(() => expect(screen.getByText("➕ Ajouter comme ami")).toBeInTheDocument(), { timeout: 3000 });
    expect(screen.getByText(/📧 Email:/i)).toBeInTheDocument();
  });

  test("supprime un ami", async () => {
    const user = await setup({ "invitations/sent": [], "invitations/received": [] });
  ...
  test("envoie une invitation depuis le modal", async () => {
    ...
    await waitFor(() => expect(fetch).toHaveBeenCalledWith(
      ...
      expect.objectContaining({ method: "POST", body: JSON.stringify({ friend_id: 1 }) })
    ), { timeout: 3000 });
  });

  test.each([
    ["accepte une invitation reçue",  /📥 Invitations Reçues \(1\)/i,   "Frank Miller", "Accepter", "invitations/accept/2", "POST"],
    ["refuse une invitation reçue",   /📥 Invitations Reçues \(1\)/i,   "Frank Miller", "Refuser",  "delete-received/2",    "DELETE"],
    ["annule une invitation envoyée", /📤 Invitations Envoyées \(1\)/i, "Emma Watson",  "Annuler",  "delete-sent/1",        "DELETE"],
