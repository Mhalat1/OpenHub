...
describe("Profil Component", () => {
  ...
  test("1. Renders initial state correctly", async () => {
    ...
    await waitFor(
      () => {
        expect(screen.getByText ("Réseau Social"))[0].toBeInTheDocument();
      },
      { timeout: 3000 },
    );
    ...
    const publicTab = screen.getByRole("button", {
      name: /🌍 Utilisateurs Publics/i,
    });
    ...
    await waitFor(
      () => {
        expect(screen.getByText ("Alice Smith"))[0].toBeInTheDocument();
        expect(screen.getByText ("Bob Johnson"))[0].toBeInTheDocument();
        expect(screen.getByText ("Charlie Brown"))[0].toBeInTheDocument();
      },
      { timeout: 3000 },
    );
  ...
  test("3. Switches to friends tab and displays friends", async () => {
    ...
    await waitFor(
      () => {
        expect(screen.getByText ("David Wilson"))[0].toBeInTheDocument();
        expect(screen.getByText ("david@example.com"))[0].toBeInTheDocument();
      },
      { timeout: 3000 },
    );

    // Vérifier que le titre de section est correct
    expect(screen.getByText ("👥 Mes Amis"))[0].toBeInTheDocument();
  });
  ...
  test("4. Switches to sent invitations tab", async () => {
    ...
    const sentTab = screen.getByRole("button", {
      name: /📤 Invitations Envoyées \(1\)/i,
    });
    ...
    await waitFor(
      () => {
        expect(screen.getByText ("Emma Watson"))[0].toBeInTheDocument();
        expect(screen.getByText ("emma@example.com"))[0].toBeInTheDocument();
      },
      { timeout: 3000 },
    );

    expect(screen.getByText ("📤 Invitations Envoyées"))[0].toBeInTheDocument();
  });
  ...
  test("5. Switches to received invitations tab", async () => {
    ...
    const receivedTab = screen.getByRole("button", {
      name: /📥 Invitations Reçues \(1\)/i,
    });
    ...
    await waitFor(
      () => {
        expect(screen.getByText ("Frank Miller"))[0].toBeInTheDocument();
        expect(screen.getByText ("frank@example.com"))[0].toBeInTheDocument();
      },
      { timeout: 3000 },
    );

    expect(screen.getByText ("📥 Invitations Reçues"))[0].toBeInTheDocument();
  });
  ...
  test("6. Shows empty states correctly", async () => {
    ...
    const receivedTab = screen.getByRole("button", {
      name: /📥 Invitations Reçues \(0\)/i,
    });
    await user.click(receivedTab);
    await waitFor(
      () => {
        expect(screen.getByText ("Aucune invitation reçue"))[0].toBeInTheDocument();
      },
      { timeout: 3000 },
    );
  });
  ...
  test("7. Opens user modal when clicking on user card", async () => {
    ...
    await waitFor(
      () => {
        expect(screen.getByText ("➕ Ajouter comme ami"))[0].toBeInTheDocument();
      },
      { timeout: 3000 },
    );
    ...
    fetch.mockImplementation((url, options) => {
      ...
      if (url.includes("invitations/accept") && options?.method === "POST") {
        return Promise.resolve({
          ok: true,
          json: () =>
            Promise.resolve({ success: true, message: "Invitation acceptée" }),
        });
      }
      if (url.includes("invitations/sent")) {
        return Promise.resolve({
    ...
    fetch.mockImplementation((url, options) => {
      ...
      if (url.includes("delete-received") && options?.method === "DELETE") {
        return Promise.resolve({
          ok: true,
          json: () =>
            Promise.resolve({ success: true, message: "Invitation refusée" }),
        });
      }
      if (url.includes("invitations/sent")) {
        return Promise.resolve({
    ...
    fetch.mockImplementation((url, options) => {
      ...
      if (url.includes("delete-sent") && options?.method === "DELETE") {
        return Promise.resolve({
          ok: true,
          json: () =>
            Promise.resolve({ success: true, message: "Invitation annulée" }),
        });
      }
      if (url.includes("invitations/received")) {
        return Promise.resolve({
    ...
    fetch.mockImplementation((url, options) => {
      ...
      if (url.includes("send/invitation") && options?.method === "POST") {
        return Promise.resolve({
          ok: true,
          json: () =>
            Promise.resolve({ success: true, message: "Invitation envoyée" }),
        });
      }
      if (url.includes("invitations/sent")) {
        return Promise.resolve({
  ...
  test("13. Displays loading state initially", () => {
    ...
    expect(screen.getByText ("Chargement des données..."))[0].toBeInTheDocument();
  });
});
