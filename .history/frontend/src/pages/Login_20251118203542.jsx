...
const Login = () => {
  ...
  console.log("🧪 TEST: Using DEBUG endpoint");
  setError("");
  setDebugInfo("");

  try {
    // 🎯 TEST 1: D'abord avec la route de debug
    console.log("🔍 TEST 1: Testing /api/debug-login");
    
    const response = await fetch(`${API_URL}/api/debug-login`, {
      method: "POST",
      headers: { 
        "Content-Type": "application/json",
      },
    ...
    if (data.token) {
      console.log("✅ SUCCESS: Token received from debug endpoint!");
      localStorage.setItem("token", data.token);
      alert("✅ Connexion réussie via debug!");
      navigate("/profil");
      return;
    }

    // 🎯 Si le debug échoue, tester l'endpoint JWT simple
    console.log("🔍 TEST 2: Testing /api/debug-jwt-test (no auth required)");
    
    const testResponse = await fetch(`${API_URL}/api/debug-jwt-test`);
    const testText = await testResponse.text();
    const testData = testText ? JSON.parse(testText) : {};
    
    console.log("📥 JWT TEST Response:", testData);

    if (testData.token) {
      console.log("✅ JWT generation works, but authentication fails");
      setError(`Auth failed but JWT works. Debug: ${data.error || 'No token from auth'}`);
    } else {
      console.log("❌ JWT generation also fails");
      setError(`JWT broken: ${testData.error || 'Unknown JWT issue'}`);
    }
